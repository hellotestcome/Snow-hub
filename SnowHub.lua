...
local listfastattack = {'Normal Attack','Fast Attack','Super Fast Attack'}
	...
	if _G.FastAttackSnow_Mode == "Fast Attack" then
		_G.Fast_Delay = 0.12
	elseif _G.FastAttackSnow_Mode == "Normal Attack" then
		_G.Fast_Delay = 0.2
	elseif _G.FastAttackSnow_Mode == "Super Fast Attack" then
		_G.Fast_Delay = 0.05
	end
end)
...
while wait() do
    pcall(function()
        if _G.FastAttackSnow then
            repeat wait(_G.DelayAttackPlayer)
               AttackNoCoolDown()
            until not _G.FastAttackSnow
        end
    end)
end
end)
	...
	Callback = function()
		...
		for k, v in pairs(cac) do
			if ListHuhu[v] and game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Buy" .. v, true) == 1 then
				...
				huhu.Image = "rbxassetid://1000008467"
...
----------------------------------------------------------------------------------------------------------------------------------------------
InterfaceManager:SetFolder("SnowScriptHub")
SaveManager:SetFolder("SnowScriptHub/specific-game")
Window:SelectTab(1)
Fluent:Notify({
    Title = "Snow Hub",
